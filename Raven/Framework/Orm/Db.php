  ...
  function connect() {
    ...
    $this->connection = function() use(&$ce) {
      if ($ce->protocol == 'mongodb') {
        ...
        if (!$db) {
          $ce->except("<b>DB:</b> Can't connect to the database! ");
        }
      } else {
        $ce->type = 'pdo';
        try {
          ...
          $ce->except('<b>DB:</b> Can\'t connect to database ' . $ce->protocol . '!');
        }
      }
      return $db;
    };
  ...
  function exec($query, $prepare = null, $is_cmd = false) {
    ...
    if ($this->type == 'pdo') {
      ...
      if (!$result) {
       ...
       $this->except('<b>DB exec():</b> Database type "' . $this->type . '" unrecognized');
    }
    return $result;
  }
...
#    return $this->get($command, $first, $is_command = true);
  ...
  /*
    Vinculamos a DB con la libreria Tablefy para el acceso de los datos como:
  ...
  */
  ...
  function get($sql, $first = false, $prepare = null) {
    ...
    if ($this->type == 'pdo') {
      ...
      return $rp;
       ...
       $this->except('<b>DB get():</b> No conozco el tipo de base de datos "' . $this->type . '"');
    }
  }
  function map($sql, $cb) {
    $rp = $this->get($sql);
  ...
  function last_insert_id() {
    ...
    if ($this->type == 'pdo') {
      ...
      $this->except('<b>DB:</b> I don\'t know how to return the last insert ID with this DB type!');
    }
  }
